...
public class MatchFinder extends StageType {
	...
	public MatchFinder(PApplet p) {
		super(p);
		this.p = p;
		matchFinder = p.loadImage("stageImages" + Stage.fileSeparator + "Instructions.jpg");
	...
	public void setup(User u) {
		...
		for (Entry<String, Double> e : match.getValues().entrySet()) {
			if (!e.getKey().equals("attack time") && !text.contains(e.getKey())) text += e.getKey() + ": " + e.getValue() + "\n";
		}
	}
