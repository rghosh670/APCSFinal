 ...
 *
 */
public class Player3 extends Player implements Hitboxable {

	private final int N_SPRITES_X = 6;
	...
	public Player3(PApplet p) {
		this(p, 0, 0, null);
	}

	public Player3(PApplet p, int xPos, int yPos, Player opponent) {
		super(p, xPos, yPos, PlayerState.TRUMP, opponent);
		this.p = p;
		this.x = xPos + 30;
		this.y = yPos;
		...
		PImage my_sprite_sheet_right = p.loadImage("playerImages" + Stage.fileSeparator + "Trump.png");
		...
		for (int x = 0; x < N_SPRITES_X; x++) {
			my_sprites_right[index] = my_sprite_sheet_right.get(x * width, height, width, height);
			my_sprites_left[index] = my_sprite_sheet_right.get(my_sprite_sheet_right.width - x * width - width,
